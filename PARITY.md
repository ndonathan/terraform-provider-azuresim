# AzureSIM ↔ AzureRM feature parity
...
<!-- AUTO-GENERATED PARITY START -->

> This block is auto-generated by `scripts/generate-parity-report.py`. Do not edit by hand. Run the script after a resource is added or after refreshing `scripts/azurerm-resources.snapshot.json`; CI verifies the file stays in sync.

## Coverage summary

**61 / 1128 AzureRM resources implemented (5.4%).**

- AzureSIM-registered resource types: **61**
- AzureRM resource types in snapshot: **1128**
- Name-matched (1:1 `azuresim_X` ↔ `azurerm_X`): **61**
- AzureSIM-only (no AzureRM equivalent by name): **0**

_AzureRM snapshot: v4.73.0 (published 2026-05-14T17:52:27Z, captured 2026-05-18T17:01:58Z). Source: https://registry.terraform.io/v1/providers/hashicorp/azurerm._

## Recommended next-up resources

Hand-curated picks for the next round of additions. Updated alongside `BACKLOG.md` whenever priorities shift; not generated from the snapshot (the Registry API doesn't expose per-resource popularity).

| # | AzureRM resource | Rationale |
|---|---|---|
| 1 | `azurerm_linux_virtual_machine` | AzureSIM has windows_virtual_machine and the legacy generic virtual_machine but no Linux split — the most common Azure VM type in real-world modules. |
| 2 | `azurerm_lb_backend_address_pool` | AzureSIM has azuresim_lb but no children; backend pool is required for any non-trivial load balancer config. |
| 3 | `azurerm_lb_rule` | Pairs with lb_backend_address_pool and lb_probe to make the LB resource actually useful end-to-end. |
| 4 | `azurerm_dns_zone` | AzureSIM has private_dns_zone but no public DNS — common for TLS / cert-manager / ingress demos. |
| 5 | `azurerm_key_vault_certificate` | AzureSIM has key_vault_secret and key_vault_key but not certificates — required for TLS-binding scenarios. |

## Resource-level gaps (by AzureRM subcategory)

AzureRM defines **1067** resources that AzureSIM does not yet implement, spread across **114** subcategories. Grouped below by subcategory, largest first. `azuresim_*` names are inferred — verify naming before adding.
...
### Azure Managed Lustre File System (1)
...
### Service Fabric Managed Clusters (1)
...
All 61 AzureSIM resources map 1:1 by name to an AzureRM equivalent. Grouped by AzureRM subcategory for cross-reference:

- **API Management** (1): `azuresim_api_management`
- **App Service (Web Apps)** (5): `azuresim_linux_function_app`, `azuresim_linux_web_app`, `azuresim_service_plan`, `azuresim_windows_function_app`, `azuresim_windows_web_app`
- **Application Insights** (1): `azuresim_application_insights`
- **Authorization** (2): `azuresim_role_assignment`, `azuresim_user_assigned_identity`
- **Base** (1): `azuresim_resource_group`
- **CDN** (1): `azuresim_cdn_frontdoor_profile`
- **Compute** (5): `azuresim_linux_virtual_machine_scale_set`, `azuresim_managed_disk`, `azuresim_virtual_machine`, `azuresim_windows_virtual_machine`, `azuresim_windows_virtual_machine_scale_set`
- **Container** (2): `azuresim_container_registry`, `azuresim_kubernetes_cluster`
- **Container Apps** (2): `azuresim_container_app`, `azuresim_container_app_environment`
- **CosmosDB (DocumentDB)** (1): `azuresim_cosmosdb_account`
- **Database** (4): `azuresim_mssql_database`, `azuresim_mssql_server`, `azuresim_mysql_flexible_server`, `azuresim_postgresql_flexible_server`
- **Key Vault** (3): `azuresim_key_vault`, `azuresim_key_vault_key`, `azuresim_key_vault_secret`
- **Load Balancer** (1): `azuresim_lb`
- **Log Analytics** (1): `azuresim_log_analytics_workspace`
- **Messaging** (6): `azuresim_eventhub`, `azuresim_eventhub_namespace`, `azuresim_servicebus_namespace`, `azuresim_servicebus_queue`, `azuresim_servicebus_subscription`, `azuresim_servicebus_topic`
- **Monitor** (3): `azuresim_monitor_action_group`, `azuresim_monitor_diagnostic_setting`, `azuresim_monitor_metric_alert`
- **Network** (15): `azuresim_application_gateway`, `azuresim_bastion_host`, `azuresim_firewall`, `azuresim_nat_gateway`, `azuresim_network_interface`, `azuresim_network_security_group`, `azuresim_network_security_rule`, `azuresim_private_endpoint`, `azuresim_public_ip`, `azuresim_route_table`, `azuresim_subnet`, `azuresim_subnet_network_security_group_association`, `azuresim_subnet_route_table_association`, `azuresim_virtual_network`, `azuresim_virtual_network_peering`
- **Private DNS** (2): `azuresim_private_dns_zone`, `azuresim_private_dns_zone_virtual_network_link`
- **Recovery Services** (1): `azuresim_recovery_services_vault`
- **Redis** (1): `azuresim_redis_cache`
- **Storage** (3): `azuresim_storage_account`, `azuresim_storage_blob`, `azuresim_storage_container`

## Attribute-level gaps

**Deferred to a follow-up.** The Terraform Registry API exposes the resource _list_ but not per-resource attribute schemas. Surfacing AzureRM attributes that AzureSIM omits requires either (a) parsing each resource's source schema from `hashicorp/terraform-provider-azurerm/internal/services/*/`, or (b) introspecting the Registry's per-resource doc-page HTML. Both are larger pieces of work and are tracked in TODO.md.

<!-- AUTO-GENERATED PARITY END -->
